...
// Minimal type definitions for ContextEngine API (OpenClaw v2026.4.10+)
// Defined locally to avoid importing from "openclaw/*"
...
function extractUserPrompt(messages: Array<Record<string, unknown>>): string | null {
...
export function createMetaClawContextEngine(
  ...
  return {
    ...
    async ingest(_messages: Array<Record<string, unknown>>): Promise<void> {
      // No-op: turn ingestion is handled by auto-capture hook + sidecar scheduler.
    },

    async ingestBatch(_messages: Array<Array<Record<string, unknown>>>): Promise<void> {
      // No-op: same as ingest — sidecar scheduler handles batching.
    },

    async afterTurn(_result: Record<string, unknown>): Promise<void> {
      // No-op: consolidation handled by sidecar scheduler.
    },

    async assemble(query: AssembleQuery): Promise<AssembleResult> {
    ...
    async compact(_sessionId: string): Promise<CompactResult> {
      try {
